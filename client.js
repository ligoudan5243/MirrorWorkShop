...
(async function() {
    ...
    const displayInternalId = safeGet('displayInternalId'); // 新增：用于显示内部ID的纯文本元素
    const internalId = safeGet('internalId'); // 隐藏字段，存储内部ID
    ...
    if (bucketForm) {
        bucketForm.addEventListener('submit', async (e) => {
            ...
            const idValue = internalId.value.trim(); // 从隐藏字段获取内部ID
